...
class ServicioService {
  ...
  try {
    final headers = await _getAuthHeaders();
    final url = Uri.parse('$_baseUrl/api/favoritos/cliente/$clienteId/servicio/$servicioId');
...
Future<List<HorarioSemanal>> buscarHorariosPorDiaYServicio(String diaSemana, int idServicio) async {
  try {
    final token = await _prefs.token;
    // Construimos la URL con los QueryParams que pide tu controlador
    final url = Uri.parse('$_baseUrl/horarios/buscar').replace(queryParameters: {
...
Future<bool> crearReserva(int clienteId, int servicioId, int horarioId, String fecha) async {
  try {
    final token = await _prefs.token;
...
Future<Map<String, int>> obtenerPlazasDisponibles(String fecha, int horarioId) async {
  try {
    final headers = await _getAuthHeaders();
    // URL: /citas/disponible?fecha=2026-01-19&horarioId=1
    final url = Uri.parse('$_baseUrl/citas/disponible').replace(queryParameters: {
      'fecha': fecha,
