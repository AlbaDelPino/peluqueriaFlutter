...
class _MisCitasScreenState extends State<MisCitasScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Citas Próximas: Fecha es hoy o posterior
    final proximas = _citas.where((cita) {
      DateTime fechaCita = DateTime.parse(cita['fecha']);
      return fechaCita.isAfter(hoySinHora) || fechaCita.isAtSameMomentAs(hoySinHora);
    }).toList();

    // Citas Pasadas: Fecha es anterior a hoy
    final pasadas = _citas.where((cita) {
      DateTime fechaCita = DateTime.parse(cita['fecha']);
      return fechaCita.isBefore(hoySinHora);
    }).toList();
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          ...
          bottom: TabBar(
            indicatorColor: naranjaBernat,
            labelColor: naranjaBernat,
        ...
        body: _isLoading
            ...
            : TabBarView(
                children: [
                  _buildTabList(proximas), // Pestaña de Próximas
                  _buildTabList(pasadas),  // Pestaña de Historial
                ],
              ),
      ),
    );
  ...
  // Widget para generar la lista dentro de cada pestaña
  Widget _buildTabList(List<dynamic> listaFiltrada) {
    return RefreshIndicator(
      ...
      child: listaFiltrada.isEmpty
          ...
          : ListView.builder(
              ...
              itemBuilder: (context, index) => _buildModernCitaCard(listaFiltrada[index]),
            ),
    );
  }

  Widget _buildEmptyState() {
    return SingleChildScrollView( // Para que el RefreshIndicator funcione aunque esté vacío
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const TextoAutomatico("No hay citas en esta sección", 
